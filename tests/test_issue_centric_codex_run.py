...
class FetchNextPromptCodexRunIntegrationTests(unittest.TestCase):
    def test_fetch_next_prompt_executes_codex_run_and_records_trigger_comment(self) -> None:
        state = {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                with self.assertRaisesRegex(BridgeStop, "trigger comment: https://github.com/example/repo/issues/20#issuecomment-701"):
            ...
            self.assertEqual(saved["last_issue_centric_launch_status"], "not_implemented")
    ...
    def test_fetch_next_prompt_records_blocked_codex_run_reason(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                with self.assertRaisesRegex(BridgeStop, "codex_run の最小 execution slice まで実行しました"):
                    fetch_next_prompt.run(dict(state), [])
