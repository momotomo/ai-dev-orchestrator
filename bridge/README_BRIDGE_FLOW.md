...
- Safari fetch 待機は通常運用で 1800 秒前提。通常ケースでは追加対処なしで進めてよい
...
- 入口は `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6`
- 初回だけ、ユーザーが ChatGPT に送る本文を入力する
- この入力本文が初回 request の正本で、bridge はそのまま送る。内部 state や古い request 材料から補わない

### 通常継続

- `ready_for_codex`、`codex_done`、`idle + need_chatgpt_next=true` など通常継続中は、同じ `run_until_stop.py` を再実行してよい
- `max_execution_count` は上限なので、途中で完了や人確認待ちになれば、そこまでで正常停止しうる
- 既存チャットをそのまま使い、新しいチャット自動作成には寄せない

### handoff 再開

- `human_review` と `need_info` は blocked / error ではないので、同じ `run_until_stop.py` からそのまま再開してよい
...
- 最初から prompt request をやり直すなら、通常入口として `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` に戻してよい
...
1. `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を実行する
2. bridge が初回だけ、ChatGPT に送る本文の入力を求める。表示される例文をもとに本文を入力すると、その本文をそのまま送信する
...
- 通常運用は `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` から始めてよい
- 初回だけ、ユーザーが入力した本文を ChatGPT へそのまま送る。初回 request のソースオブトゥルースはその入力本文
...
- 通常表示では `mode` や flag を前面に出さず、`初回入力待ち`、`ChatGPT返答待ち`、`Codex実行中`、`人確認待ち`、`異常` などの人向け表示を優先する
...
- `human_review` と `need_info` は blocked / error ではなく、通常入口の `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` からそのまま再開してよい
...
- 初回起動は `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を起点にし、表示された例文をもとに最初の request 本文を入力する
...
- `python3 scripts/bridge_orchestrator.py --project-path /path/to/target-repo`: 1 手ずつ詳細確認したい時だけ使う。通常入口は `run_until_stop.py`
...
- 1 回目起動では `run_until_stop.py` が初回本文入力を受け、その本文をそのまま ChatGPT へ送り、request / fetch を含めて進む
...
- `idle + need_chatgpt_prompt=true`: 通常は `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を実行する。1 手ずつ確認したい時だけ `python3 scripts/bridge_orchestrator.py --project-path /path/to/target-repo` を使う
...
- Codex 実行方法を変えたい場合は `codex_bin`、`codex_model`、`codex_timeout_seconds` を変える
...
3. Safari current tab、Automation、`state.json` 前提を確認して、`python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を実行し、表示された例文をもとに初回本文を入力する
...
- `bridge/browser_config.json` で変えるもの: `fetch_timeout_seconds`、`poll_interval_seconds`、`apple_event_timeout_retry_count`、`apple_event_timeout_retry_delay_seconds`、`runner_heartbeat_seconds`、必要なら `chat_hint`
...
- 初回 request 本文は `project_config.json` から自動生成しない。起動時のユーザー入力本文をそのまま使う
