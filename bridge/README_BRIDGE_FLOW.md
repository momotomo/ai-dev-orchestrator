...
この文書は current runtime の runbook であり、repo 全体の source of truth
再定義そのものではない。issue-centric 段階移行の正本は
`docs/ISSUE_CENTRIC_FLOW.md` を参照する。通常運用では ready issue を実行単位
の正本へ寄せていく。現行 runtime では ready issue 参照の初回入口が使えるよう
になったが、free-form override 経路もまだ残る。runtime 側でどこから先が将来
の実装境界になるかは
`docs/RUNTIME_TOUCHPOINT_INVENTORY.md` を参照する。bridge / ChatGPT / Codex
間の合意済み contract は
`docs/ISSUE_CENTRIC_RUNTIME_CONTRACT.md` を参照する。ChatGPT reply の取得は
まだ visible text ベースなので markdown source fidelity は保持されない。
copy-response 経路の feasibility verdict は
`docs/MARKDOWN_FIDELITY_FEASIBILITY.md` を参照する。
...
- 通常運用では、最初に current open `ready` issue を確認する
- current open `ready` issue が 1 件あるなら、それを今回の実行単位正本として見る
- current open `ready` issue が 0 件なら、`planned` backlog を見直して次の bounded `ready` issue を 1 件だけ選ぶ
- 現行 runtime では `--ready-issue-ref`、または起動直後の 1 行入力で current ready issue の参照を受けられる
- free-form 初回本文は通常入口そのものではなく、選んだ `ready` issue を ChatGPT へ向けて指し示すための runtime override 経路として扱う
- 自由入力を、open `ready` がある通常入口の代替として使わない
...
## 例外 / recovery / override として自由入力を使ってよい最小ケース

- backlog がまだ整っておらず、次の issue を切る前に 1 回だけ探索したい時
- 緊急の一点修正を 1 フェーズだけ先に確認したい時
- `human_review`、`need_info`、stale runtime、送信失敗などの運用異常から短い補足で回復したい時
- 現行 runtime を intended `ready` issue か backlog home へ戻すために、一時的な override が必要な時

この経路を使う時も:

- 依頼文は短く保ち、1 フェーズを超えない
- open `ready` issue があるなら必ずそれを指す
- open `ready` issue が無いなら、戻り先にする `planned` issue か backlog home を文中で明示する
- 自由入力そのものを長期の task 定義にしない。実施後は issue / backlog 側へ戻す
...
- 迷った時は `python3 scripts/start_bridge.py --status` で、内部 JSON を開かずに「新規開始 / 続きから再開 / 人確認待ち / 先に確認」のどれかを確認できる
- 続きからそのまま進めたい時は、同じ `python3 scripts/start_bridge.py ...` を再実行すればよい。明示したい時だけ `--resume` を付けてよい
...
- そのまま続けてよい時は `python3 scripts/start_bridge.py --resume --project-path /path/to/target-repo --max-execution-count 6`
- `human_review` / `need_info` で止まった時も、同じ `--resume` で補足入力へ進めてよい
- handoff 再送待ちや Safari 由来の bridge error で止まった時は、まず `python3 scripts/start_bridge.py --clear-error --project-path /path/to/target-repo --max-execution-count 6`
- 手動 pause、`bridge/STOP`、stale 候補、未退避 report など「先に見る」べき停止は `python3 scripts/start_bridge.py --doctor --project-path /path/to/target-repo --max-execution-count 6`
- stop 後 summary に `おすすめ 1 コマンド` が出ている時は、それを最優先で見る。内部 state や JSON は必要時だけ確認する
- `clear-error` / `--reset` は何でも初期化しない。prompt / report / handoff / logs は残し、bridge 側の停止要因だけを最小解除する
- `doctor` が `clear-error 可能` と出た時だけ `clear-error` を使い、`bridge/STOP` / pause / stale 候補 / 未退避 report の時は先に `doctor` 側の案内を優先する
...
- 通常表示では `mode` や flag を前面に出さず、`ready issue参照で開始待ち`、`ChatGPT返答待ち`、`Codex実行中`、`人確認待ち`、`異常` などの人向け表示を優先する
...
- bridge が止まったら、内部 state を追う前に `next step:` 行と summary の `suggested_next_note` を先に見る
...
- 初回 request は起動時のユーザー入力本文が runtime 入力正本のままで、再開時はそれを上書きせず、次の ChatGPT request に添える補足だけを入力する
...
```text
#123 sample browser の apply action 文言整理
```

## free-form override の短い例
...
- 初回起動は `python3 scripts/start_bridge.py --project-path /path/to/target-repo --ready-issue-ref '#123 short title' --max-execution-count 6` を起点にしてよい。引数を省略した時は current ready issue の参照入力を求め、さらに必要な時だけ override 本文入力へ進む
