...
- `python3 scripts/bridge_orchestrator.py --project-path /path/to/target-repo`: 初回 request を 1 手ずつ確認したいときに使う。初回だけ本文入力を求める
...
- `idle + need_chatgpt_prompt=true`: `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を起動するか、1 手ずつ進めるなら `python3 scripts/bridge_orchestrator.py --project-path /path/to/target-repo` を実行する
