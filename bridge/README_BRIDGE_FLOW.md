...
の正本へ寄せていくが、現行 runtime では初回本文入力と override 経路がまだ残
る。runtime 側でどこから先が将来の実装境界になるかは
...
- 現行 runtime の初回本文入力は通常入口そのものではなく、選んだ `ready` issue を ChatGPT へ向けて指し示すための runtime 入力 / override 経路として扱う
...
- 初回だけ、bridge が短い例文を表示して本文入力を求める。この入力本文が初回 request の runtime 入力正本で、bridge は本文を改変せず固定の返答契約だけを追記して ChatGPT へ送る
...
- この入力本文が初回 request の runtime 入力正本で、bridge は本文を改変しない。内部 state や古い request 材料から補わず、送信直前に固定の返答契約だけを追記する
...
- 1 回目起動では `run_until_stop.py` が初回本文入力を受け、その本文を runtime 入力正本として固定の返答契約を追記して ChatGPT へ送り、request / fetch を含めて進む
