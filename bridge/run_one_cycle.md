...
3. ChatGPT の返答が出たら、対象チャットが Safari の現在タブのままになっていることを確認し、`python scripts/run_one_cycle.py` をもう一度実行する
...
8. さらに `python scripts/run_one_cycle.py` を実行すると、`idle + need_chatgpt_next=true` なら `request_prompt_from_report.py` が走り、Safari の現在タブにある対象チャットへ次フェーズ用の prompt request を送る
