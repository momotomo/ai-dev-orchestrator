...
2. bridge が初回だけ、ChatGPT に送る本文入力を求める。表示された例文をもとに本文を入力すると、その本文をそのまま送る
...
- 推奨: `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6`
...
- 通常運用では内部 `mode` 名より、`初回入力待ち`、`ChatGPT返答待ち`、`Codex実行待ち`、`Codex実行中`、`人確認待ち`、`異常` といった表示を優先して見る
...
- `run_until_stop.py` が止まったら、まず `handoff:` 行と summary の `suggested_next_note` を見て、内部 state は必要な時だけ開く
- `human_review` は人判断待ち、`need_info` は追加入力待ち、blocked は bridge 側事情で自動継続しない停止として見る

## `human_review` / `need_info` の再開

- どちらも blocked / error ではないので、通常入口の `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` からそのまま再開してよい
...
- Codex CLI の呼び方を変えたい場合は `codex_bin`、`codex_model`、`codex_timeout_seconds` を直す
...
- 初回 request 本文は config や state から自動生成せず、起動時のユーザー入力本文をそのまま使う
- `bridge/browser_config.json` は Safari 側の調整値を置く。`fetch_timeout_seconds`、`poll_interval_seconds`、`apple_event_timeout_retry_count`、`apple_event_timeout_retry_delay_seconds`、`runner_heartbeat_seconds`、必要なら `chat_hint` を持つ
...
1. `idle + need_chatgpt_prompt=true`
   `python3 scripts/run_until_stop.py --project-path /path/to/target-repo --max-execution-count 6` を起動するか、1 手ずつ進めるなら `python3 scripts/bridge_orchestrator.py --project-path /path/to/target-repo` を実行する。初回だけ本文入力を求める
