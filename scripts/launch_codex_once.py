...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--codex-bin", default=DEFAULT_CODEX_BIN, help="Codex CLI コマンド")
    parser.add_argument("--model", default="", help="Codex CLI に渡す model 名")
    parser.add_argument("--timeout-seconds", type=int, default=DEFAULT_TIMEOUT_SECONDS, help="Codex 実行の最大秒数")
...
def build_launch_prompt(template_path: Path, prompt_path: Path, report_path: Path) -> str:
...
def run(state: dict[str, object], argv: list[str] | None = None) -> int:
    ...
    launch_prompt = build_launch_prompt(template_path, prompt_path, report_path)
    stamp = now_stamp()
