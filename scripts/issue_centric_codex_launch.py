...
def build_issue_centric_codex_prompt(
    ...
    lines = [
        ...
        "## Local Verification",
        "",
        "実装後、以下の順で repo の標準確認を実行してください。",
        "",
        "- 各コマンドは 1 つずつ実行し、結果を確認してから次へ進む",
        "- 少なくとも存在する範囲で test / lint / build または typecheck 相当を実行する",
        "- 確認が失敗した場合は、今回 scope 内で最小修正できるものだけ直す",
        "- scope 外・重い調査・CI 環境依存の問題は広く直しに行かず、その場で止めて報告する",
        "",
        "## Report Handoff",
        "",
        ...
        "- report に以下を必ず含める: changed files / commit SHA / 実行した確認 / 未実行の確認 / local verification: passed|failed|partial / ci status: success|pending|failure|unknown / ci run id / ci run url / ci workflow / ci conclusion",
        "- CI run がまだ見つからない場合は、ci run id: none / ci run url: none / ci status: unknown と書く",
        "",
        "## 追加確認 docs",
        "",
    ]
