...
DEFAULT_STATE: dict[str, Any] = {
    ...
    "last_send_retry_prepared_request_status": "",
    ...
    "last_ci_gate_workflow": "",        # workflow name of the last CI run
...
def state_snapshot(state: Mapping[str, Any]) -> str:
    fields = [
        ...
        f"- last_send_retry_count: {state.get('last_send_retry_count', 0)}",
        f"- last_send_retry_elapsed_seconds: {state.get('last_send_retry_elapsed_seconds', 0)}",
        f"- last_send_retry_route: {state.get('last_send_retry_route', '')}",
        f"- last_send_retry_prepared_request_status: {state.get('last_send_retry_prepared_request_status', '')}",
        f"- last_send_retry_reason: {state.get('last_send_retry_reason', '')}",
        f"- last_send_retry_status: {state.get('last_send_retry_status', '')}",
...
def build_request_context_section(state: Mapping[str, Any]) -> str:
    ...
    * **contract correction context** — reminds ChatGPT to return a correct
    ...
    if ci_conclusion:
        ...
        ci_workflow = str(state.get("last_ci_gate_workflow", "")).strip()
        ci_status = str(state.get("last_ci_gate_status", "")).strip()
        ci_failure_detail = str(state.get("last_ci_gate_failure_detail", "")).strip()
        url_part = f" ({ci_run_url})" if ci_run_url else ""
        workflow_part = f" workflow={ci_workflow}" if ci_workflow else ""
        status_part = f" status={ci_status}" if ci_status else ""
